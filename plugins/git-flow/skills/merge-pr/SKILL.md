...
description: >-
  ...
  ローカルの後始末までを行う。承認は1回で、マージとブランチ削除をまとめて実行する。
  **コミットと PR 作成は担当しない。**
...
- **承認は1回だけ。** 手順5でマージとブランチ削除をまとめて提示し、そこで承認を取る。
  マージした後、削除のために改めて確認を取らない。承認を2回に割ると、**マージだけ済んで
  残骸が残った状態でユーザーの返事待ちになる**——いちばん中途半端な止まり方で、
...
**この提示への承認は、マージとブランチ削除の両方に対する承認。** 「消えるもの」を
ここに並べているのはそのため。承認が返ったら手順6〜7を最後まで通し、削除の手前で
もう一度聞き返さない。
...
```bash
git show-ref --verify --quiet refs/heads/<作業ブランチ> && echo "残っている" || echo "消えた"
```

消えていれば終わり。残っていたら、次の順で消す。

1. **`git branch -d <作業ブランチ>`。** merge commit で取り込まれていればこれで消える。
   未マージのコミットが載っていれば git が拒むので、取りこぼしを踏まずに済む
2. **`-d` が拒んだら、squash / rebase マージを疑う。** どちらも base には別のコミットとして
   入るため、git は元ブランチをマージ済みと認識しない。この場合だけ `-D` を使うが、
   **撃つ前に、ローカルの先端が実際にマージされた PR の head と同じかを確かめる。**

   ```bash
   gh pr view <番号> -R <owner/repo> --json headRefOid --jq .headRefOid
   git rev-parse <作業ブランチ>
   ```

   一致すれば、そのブランチに載っている変更は全部 base に入っている。`-D` は
...
| 見たもの | 値 |
|---|---|
| `branch.<name>.remote` / `.merge` | 設定は残っている |
| `refs/remotes/origin/<name>` | `fetch --prune` 済みで消えている |
| `git for-each-ref` の `%(upstream)` / `%(upstream:track)` | **どちらも空** |
