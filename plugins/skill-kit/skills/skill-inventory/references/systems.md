...
比較対象のパスは marketplace.json から取る。`skills` 配列があればそれを、無ければ
`source` を丸ごと使う。**配列が無いほうが新しい形。** `source: "./plugins/<name>"` の
ように配布単位をフォルダで区切っているリポジトリでは配列そのものが要らず、
`source` を見たほうが `plugin.json` の変更まで拾えて正確になる。
...
2. **`source: "./"` のとき、キャッシュ配下の `skills/` を数えても意味がない。**
   リポジトリ全体が展開されるので、生えないスキルのディレクトリまで並ぶ。実測では
   `git-flow` のキャッシュに 9 個のディレクトリがあり、実際に生えるのは
   `marketplace.json` の `skills` が選んだ 4 個だけだった。

   **ただし CLI に限る。** claude.ai とデスクトップアプリはこの配列を読まず、
   プラグインルート直下の `skills/` を総なめする。実測では、宣言 7 本のプラグインが
   デスクトップでは 15 本生えていた（`source: "./"` でリポジトリ全体が配られ、
   他プラグインのスキルまで同じ名前空間に入った）。つまり**同じ marketplace.json から
   系統ごとに違う結果が出る**。配列に頼っている構成を見たら、この食い違いを疑う。
   `source` をプラグインごとのサブディレクトリに切ると、どの実装でも同じ結果になる。
