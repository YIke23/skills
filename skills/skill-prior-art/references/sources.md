...
0=候補なし / 1=候補あり。照合するのは SKILL.md のフロントマター（`name` と
`description`）だけで、本文は見ない。

スクリプトが探す場所は4つ。無い場所は黙って飛ばす。

| 場所 | 中身 |
|---|---|
| カレントリポジトリの `skills/` | 自作の実体 |
| `~/.claude/skills` | 作業場。定常状態では空 |
| `~/.claude/plugins/cache/<marketplace>/<plugin>/<sha>/skills` | プラグインの実体。**同じプラグインの古い sha が並ぶ** |
